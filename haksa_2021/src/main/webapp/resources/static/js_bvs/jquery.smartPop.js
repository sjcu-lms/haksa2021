...
$(document).ready(function() {
	...
	$.smartPop = {
		...
		open : function(options) {
			...
			var tt = this;
			if(isapple || isios || isipad ){
				$(window).bind('resize',function(){
					tt.resizeHeight(tt.opts.height);
				});
			}
		...
		resizeHeight : function(h) {
			...
			// 위치설정
			if(this.opts.position == 'center'||this.opts.position=='') {
				var t;
				if(this.opts.isInner == 'Y'){
					t = (h < innerH) ? ((innerH - h) / 2)/2 : 10;
				}else{
			...
			if(this.opts.url == '') {
				...
				//alert((this.opts.width - this.opts.padding * 2));
				//alert((h - this.opts.padding * 2));
				var w = "";
				if(isapple || isios || isipad ){
					if(isNaN((this.opts.width - this.opts.padding * 2))){
						if(Math.round($(window).width()*(parseInt(this.opts.width.replace(/\%/g))/100)) < 354){
							w = "354px";
						}else{
							w = Math.round($(window).width()*(parseInt(this.opts.width.replace(/\%/g))/100))+"px";
						}

						//alert(w);
					}else{
						w = (this.opts.width - this.opts.padding * 2) + 'px'
						//alert(w);
					}
				}else{
					w = (this.opts.width - this.opts.padding * 2) + 'px'
				...
				$('#smartPop_frame').css({ padding: this.opts.padding + 'px', width: w, height: (h - this.opts.padding * 2) + 'px' }).show();
			}
		...
		close : function() {
			...
			$('body').css({overflow: 'auto', overflowY: 'scroll'});
			$('.wrap').css({ overflowY: ''});
		//	$('body').css({ marginRight: 0, display: '', overflowY: 'auto'});
//			$('body').css({ marginRight: 0, display: '', overflowY: 'scroll'});
