...
var fileBuffer=[];
function fn_checkAttfile(obj,limit,ul_file){
	var temp_files = $(obj).get(0).files;
	Array.prototype.push.apply( fileBuffer, temp_files ); // <-- here
	var new_file_cnt = $(temp_files).length;
	var old_file_cnt = 0;
	if($("#"+ul_file).length>0){
		old_file_cnt = $("#"+ul_file).find("li").length;
	}else{
		old_file_cnt = $(obj).parent().parent().find(".file_list li").length;
	}
	var names = [];
	var sizes = [];
    ...
    for(var j = 0; j < names.length; j++){
    	...
    	var att_file = "<li>"+ext_img+names[j]+" ( "+sizes[j]+" )<button type=\"button\" onclick=\"fn_deleteAttFile($(this).parent())\">삭제</button></li>"

    	if($("#"+ul_file).length>0){
    		 $("#"+ul_file).append(att_file);
    	}else{
    		$(obj).parent().parent().find(".file_list").append(att_file);
    	}
    }
...
function fn_xhrSubmit(action,obj_form){
	...
	for (var i =0 ; i < fileBuffer.length; i++) {
		var file = fileBuffer[i];
		formData.append("p_upfile", file);
	}
	if(fileBuffer.length==0){
		formData.append("p_upfile", '');
	}
...
function insertSystemMile(ml_no){
	var formData = new FormData();
	formData.append("p_ml_no",ml_no);
	var xhr = new XMLHttpRequest();
 	xhr.open('POST', '/servlet/controller.library.MileageSystemServlet', true);
    xhr.responseType = 'text';
    xhr.send(formData);
}

function go_jandimsg(v_jandi_url, v_liveid) {
	if( v_jandi_url == "" || v_jandi_url == null ) {
		alert("2018학년도 봄학기는 2018월 3월 2일 이후 오픈합니다.");
		return;
	} else {
		if( v_liveid == "" || v_liveid == null ) {
			//alert("2017년 2월 27일 이후 수강신청 하신 분은 3월 10일 12시부터 이용 가능하십니다.");
			//alert("LIVE 강의실 ID가 생성되지 않았습니다.\n\n 수강신청 변경기간(개강이후)에 수강신청하신 학우님\n\n 대학원 선수과목 및 TOEIC 신청학우님\n\n 세종대학교 학점교류학우님\n\n 들께서는 수강변경기간이 끝나고 추후 반영될 예정입니다.");
			alert("LIVE 강의실 ID가 생성되지 않았습니다.\n\n 수강신청 변경기간 이후에 추가로 수강신청하신 학우님들께서는\n\n 10월 초 반영될 예정입니다.");
			window.open(v_jandi_url, "_blank");
		} else {
			//alert("2017년 2월 27일 이후 수강신청 하신 분은 3월 10일 12시부터 이용 가능하십니다.\n\n LIVE 강의실 ID:"+v_liveid+"  (초기패스워드 생년월일 6자리) \n\n ※ 패스워드를 8자리 이상 변경 필수.  \n\n\n Internet Explorer 버전 10이상 안전하게 이용하실수 있습니다. \n\n 만약 버전이 10버전 이하일 경우 크롬 브라우져를 사용하시면 원할한\n\n 사용을 하실수 있습니다.  ");
			insertSystemMile(1);
			alert("LIVE 강의실 ID:"+v_liveid+" \n\n(모든사용자 초기패스워드 sjcu(본교대학원생 sjcg)+생년월일 6자리 : 총 10자리 ) \n\n ※ 패스워드를 8자리 이상 변경 필수.  \n\n 기존사용자의 경우도 패스워드가 초기화 됩니다. \n\n\n 반드시 크롬(Chrome) 브라우져를 사용하시기 바랍니다.");
			window.open(v_jandi_url, "_blank");
		}
	}
}

function go_jandidownload() {
	alert("메뉴얼을 준비중입니다.");
	return;
}
